...
- (CTDownloadWithSession *)getDownloadToolFromTempArray:(NSString *)urlStr{
    ...
    CTDownloadWithSession *request = nil;
    for (CTDownloadWithSession *req in imageShowInstance.requestArray) {
        if ([req.downloadURl.absoluteString isEqualToString:urlStr]) {
            request = req;
            break;
        }
    }
